...
const ITEMS_PER_PAGE = 6; // Frontend: 6 cards per page
const BACKEND_LIMIT = 15; // Backend: Fetch 15 entries per request
...
const Projects = () => {
  ...
  // Calculate total frontend pages
  const totalPages = Math.ceil(totalProjects / ITEMS_PER_PAGE);

  // Calculate which backend page is needed for the current frontend page
  const getBackendPage = (frontendPage) => Math.ceil((frontendPage * ITEMS_PER_PAGE) / BACKEND_LIMIT);

  useEffect(() => {
    const requiredBackendPage = getBackendPage(currentPage);
    if (requiredBackendPage !== backendPage && !loading) {
      setLoading(true);
      setBackendPage(requiredBackendPage);
    }
  }, [currentPage, backendPage, loading]);

  useEffect(() => {
    const fetchProjects = async () => {
      try {
        console.log(`Fetching backend page ${backendPage} with limit ${BACKEND_LIMIT}`);
        const res = await fetch(`/api/projects?page=${backendPage}&limit=${BACKEND_LIMIT}`);
        if (!res.ok) {
        ...
        setCachedProjects((prev) => {
          // Merge new projects, avoiding duplicates by _id
